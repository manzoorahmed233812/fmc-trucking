...
const EditLocation = () => {
	...
	const saveLoadToDatabase = async () => {
		...
		if (isLoading == true) {
			...
			if (!formik.values.locationAddress || formik.values.locationAddress == '') {
				showNotification(
					<span className='d-flex align-items-center'>
				...
				// Edit and save document in collection "Locations"
				await setDoc(doc(firestoredb, 'Locations', result[4]), locationInformation)
					.then(async (docRef) => {
						console.log('Location has been updated successfully');
						console.log(docRef);
						setisLoading(false);
					})
					.catch((error) => {
	...
	const getLocationData = async () => {
		...
		const docRef = doc(firestoredb, 'Locations', result[4]);
		const docSnap = await getDoc(docRef);

		if (docSnap.exists()) {
			console.log('Document data:', docSnap.data());
			setLocationData({ id: docSnap.id, data: docSnap.data() });
		} else {
	...
	useEffect(() => {
		if (locationData) {
			...
			//Carrier Information
			formik.values.locationAddress = locationData.data.locationAddress;
			formik.values.locationType = locationData.data.locationType;
			formik.values.locationName = locationData.data.locationName;
			formik.values.locationCode = locationData.data.locationCode;
			formik.values.telephone = locationData.data.telephone;
			formik.values.contactName = locationData.data.contactName;
			formik.values.telephoneOptional = locationData.data.telephoneOptional;
			formik.values.email = locationData.data.email;
			formik.values.fax = locationData.data.fax;
			formik.values.privateNotes = locationData.data.privateNotes;
			formik.values.publicNotes = locationData.data.publicNotes;
